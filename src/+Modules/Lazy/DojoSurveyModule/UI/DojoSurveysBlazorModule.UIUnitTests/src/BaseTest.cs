...
public class BaseTest
{
    ...
    public BaseTest()
    {
        ...
        var mockLocalizeModule = Mock.Create<IStringLocalizer<DojoSurveysCoreLanguageBase>>();
        ctx.Services.AddSingleton<IStringLocalizer<DojoSurveysCoreLanguageBase>>(mockLocalizeModule);

        var mockAllDojoSurveysPageLocalizer = Mock.Create<IStringLocalizer<AllDojoSurveys>>();
        ctx.Services.AddSingleton<IStringLocalizer<AllDojoSurveys>>(mockAllDojoSurveysPageLocalizer);

        var mockAllDojoSurveysComponentLocalizer = Mock.Create<IStringLocalizer<AllDojoSurveys>>();
        ctx.Services.AddSingleton<IStringLocalizer<AllDojoSurveys>>(mockAllDojoSurveysComponentLocalizer);

        // ------------------------------------

        // DojoSurveyModule
        var mockIDojoSurveysDataService = Mock.Create<IDojoSurveysDataService>();
        ctx.Services.AddSingleton<IDojoSurveysDataService>(mockIDojoSurveysDataService);

        var mockDojoSurveysHttpClientFactory = Mock.Create<DojoSurveysHttpClientFactory>();
        ctx.Services.AddSingleton<DojoSurveysHttpClientFactory>(mockDojoSurveysHttpClientFactory);
        // \DojoSurveyModule       
